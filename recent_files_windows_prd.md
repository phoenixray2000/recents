...
| 收藏抽屉 (Drawer) | 收藏的文件和文件夹 | **右侧展开**，显示独立持久化的收藏清单。支持「编辑模式」：常态下仅显示图标与名称以节省空间（宽度 160px），编辑模式下显示拖拽把手与取消固定按钮。 |
...
- **收藏独立逻辑与编辑模式**：收藏夹逻辑与主列表解耦。收藏项支持「编辑模式」切换：
    - **常规模式**：仅显示图标和名称，禁止内部排序拖拽，鼠标移动触发外部文件拖拽（`DataFormats.FileDrop`）。
    - **编辑模式**：显示左侧拖拽把手（Drag Handle）和右侧取消固定按钮。**只有拖拽把手能触发内部排序拖拽，点击文件名区域不触发任何拖拽**；此时外部拖拽禁用。拖拽过程中，鼠标旁显示文件名浮层（Adorner），列表中显示蓝色插入线指示目标位置。
    - **宽度优化**：收藏栏宽度由 280px 降至 160px，总窗口宽度相应增加 160px（由 `UpdateWindowWidth()` 原子管理，同时控制抽屉可见性与窗口宽度，避免闪烁）。
    - **图标缓存**：收藏时同步捕获系统图标并以 PNG 字节存入 `favorites.icon_png`；下次启动直接从数据库解码，无需等待 Shell 查询（见 §6.23）。
    - **缺失文件标注**：收藏的文件若不存在，以红色删除线样式标注，不从收藏列表移除。
...
- 例外：收藏文件 (Favorites) **跳过系统/隐藏文件的属性过滤规则**，但**仍受顶部 Chip 类型筛选约束**（例如：在 "Docs" Chip 下，收藏的图片不会出现）。不增加额外覆盖规则。
...
CREATE TABLE favorites (
    ...
    icon_cache_key       TEXT,
    last_seen_time       INTEGER NOT NULL,
    icon_png             BLOB               -- 收藏时捕获的系统图标 PNG 字节，避免每次启动重新从 Shell 获取
);
...
- **存在性表现**：收藏文件不存在时，以**红色删除线**样式标注，不从收藏列表移除（与主列表的 Missing 灰显不同，收藏夹用更醒目的视觉提示）。应用启动时所有收藏项的 `exists_state` 重置为 `Unknown`，由 `ExistsProbeService` 异步重新探测；探测到 `Missing` 后仅刷新显示状态，不执行 `RemoveAsync`。
- **图标缓存**：收藏操作触发时，异步捕获该文件的系统图标（`SHGetFileInfo` / `IShellItemImageFactory`），编码为 PNG 字节存入 `favorites.icon_png` 列。下次启动直接从数据库字节解码，跳过 Shell 查询，确保收藏栏图标即时显示。若文件已缺失导致图标捕获失败，使用上次缓存的图标。
- **取消固定**：支持在主列表、左侧视图或右侧抽屉中取消固定。编辑模式下，每个收藏项右侧显示取消固定按钮（蓝色星形图标）。
- **重排序（编辑模式）**：右侧抽屉标题栏提供 Edit/Done 切换按钮。编辑模式下每个收藏项左侧显示拖拽把手（`≡` 图标，`Segoe Fluent Icons `）；**仅拖拽把手可触发内部排序，点击文件名区域不触发拖拽**；拖拽过程中实时显示蓝色插入线标记目标位置，鼠标旁显示文件名浮层（DragAdorner）。顺序持久化保存到 `favorite_order` 字段。
- **外部拖入**：常规模式下支持将外部文件（如从资源管理器或其他应用）直接拖入收藏栏（抽屉）进行收藏；编辑模式下禁用外部拖入，专用于内部排序。
...
- **文件操作后自动隐藏**：执行打开文件、打开所在位置、打开方式等操作时，以事件驱动方式隐藏主窗口：`FileActionService` / `ShellService` 的 `ActionExecuted` 事件触发后，设置 `_pendingActionHide` 标志；当目标应用（如 Explorer）获得前台焦点导致主窗口失焦时（`Deactivated` 事件），立即隐藏；同时启动 600ms 的兜底定时器，确保在 `Deactivated` 未触发时也能最终隐藏。若用户在操作后重新激活 Recents 窗口，取消兜底定时器并重置标志。此机制解决了"文件夹在后台打开"的双击焦点竞争问题。
...
- [ ] **收藏抽屉开启时，窗口宽度自动增加 160px，主列表宽度保持不变；抽屉可见性与宽度由 `UpdateWindowWidth()` 原子管理，无闪烁**。

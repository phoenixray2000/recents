...
- 主窗口暗色 UI，按 §5 和 §7 实现：
  ...
  - 左侧导航 `All Files / Favorites / Recent Folders / Documents / Images / Code`（**不显示 Settings**）
...
- 显示最近 200 个文件，时间倒序
- 搜索文件名 / 扩展名 / 路径
- 左侧导航与顶部 Chip 的叠加筛选（注：`All Files` 仅显示文件，不显示文件夹）
- 双击打开原文件
...
第一阶段**不做**：白名单 / 黑名单 UI、完整 Jump List 支持、真实缩略图。Jump List 当前明确不作为数据源。

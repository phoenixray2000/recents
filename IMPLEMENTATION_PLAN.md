...
2.  **Logic**: 捕获 `KeyDown` 事件，识别组合键（Ctrl, Alt, Shift）并格式化为字符串。

### ❌ Task-N4：列表右键菜单支持
1.  **UI**: 在 `MainWindow.xaml` 的 `ListBox` 或 `ListBoxItem` 上添加 `ContextMenu`。
2.  **Binding**: 将右键菜单项绑定到对应的 Command (Open, Reveal, Copy, etc.)。
